...
const colorsXml = require('./xml/colors.js');
const styleXml = require('./xml/style.js');
...
class XLSX {
  ...
  async addCharts(zip, model) {
    await Promise.all(
      model.charts.map(async chart => {
        const chartSpaceXform = new ChartSpaceXform();
        const relationshipsXform = new RelationshipsXform();

        let xmlStream = new XmlStream();
        chartSpaceXform.prepare(chart);
        chartSpaceXform.render(xmlStream, chart);
        zip.append(xmlStream.xml, {name: `xl/charts/${chart.name}.xml`});

        if (chart.rels && chart.rels.length) {
          xmlStream = new XmlStream();
          relationshipsXform.render(xmlStream, chart.rels);
          zip.append(xmlStream.xml, {name: `xl/charts/_rels/${chart.name}.xml.rels`});
        }

        zip.append(styleXml, {name: `xl/charts/${chart.style}.xml`});
        zip.append(colorsXml, {name: `xl/charts/${chart.colors}.xml`});
