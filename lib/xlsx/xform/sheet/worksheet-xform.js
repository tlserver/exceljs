...
class WorkSheetXform extends BaseXform {
  ...
  prepare(model, options) {
    ...
    model.charts.forEach(chart => {
      let {drawing} = model;
      bookChart = options.charts[chart.chartId];
      if (!drawing) {
        drawing = model.drawing = {
          rId: nextRid(rels),
          name: `drawing${++options.drawingsCount}`,
      ...
      let rIdChart = this.preChartId === chart.chartId ? drawingRelsHash[chart.chartId] : drawingRelsHash[drawing.rels.length];
      if (!rIdChart) {
        rIdChart = nextRid(drawing.rels);
        drawingRelsHash[drawing.rels.length] = rIdChart;
        drawing.rels.push({
          Id: rIdChart,
          Type: 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart',
          Target: `../charts/${bookChart.name}.xml`,
        });
      }
